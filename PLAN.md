...
- **It is a ~2010 snapshot.** Spot-checking about 50 area codes introduced since
  then, only a handful are present. Missing examples: 986, 640, 726, 838, 332,
  930, 934, 463, 629, 680, 279, 820, 840, 448, 656, 689, 945, 826, 943, 686,
  861, 236, 672, 431, 367, 782, 825, 548, 437, 753, 873, 564, 628, 341, 669,
  737, 346, 984, 929.
- The good news: essentially every one of those is an **overlay**, i.e. it
  shares its footprint with a code that *is* in the file (332 = 212's shape,
  984 = 919's, 929 = 718/347's, 437 = 416/647's). Geographic splits have been
  rare since 2010. So the geometry is still right; only the labelling is stale.

### Decision: keep the shapes, replace the metadata
...
- **Geometry**: run the small GeoJSON through `mapshaper` (simplify ~5–10%,
  ...
  stable `shapeId` (use the pre-overlay NPA, e.g. `212`). Target ≤ 400 KB
  gzipped. Checked into `src/data/shapes.topo.json`, regenerated by a script.
- **Area code metadata**: a separate `src/data/areacodes.json`, one row per
  current NPA, generated from the NANPA public NPA report (a downloadable CSV
  listing every NPA with location, in-service date, overlay complex and
  status). Fields we keep: `npa`, `region` (state/province/country), `country`,
  `inService` (year), `overlayComplex` (list of sibling NPAs), `cities`
  (short curated list, hand-maintained), and `shapeId` (the join key into the
  TopoJSON).
- **Join rule**: `shapeId` = the oldest NPA in the overlay complex that exists
  in the shape file. A build-time check fails if any active NPA has no shape,
  so new overlays get caught when the metadata is refreshed.
- **Refresh**: `npm run data:build` re-downloads the NANPA CSV, re-runs the
  join, and prints anything unmapped. Refreshing shapes is a manual step that
  we expect to do rarely (only when a true geographic split happens).
...
| Tests | **Vitest** for parsing/encoding logic, **Playwright** for one import-to-map smoke test | The parsing code is where bugs will hide |
...
Node 22, npm (drop yarn v1 and the nix/direnv files unless you use them elsewhere).

## Repository layout (target)

```
scripts/
  build-shapes.mjs        mapshaper → TopoJSON, assigns shapeId
  build-areacodes.mjs     NANPA CSV → areacodes.json, validates join
src/
...
- `build-shapes.mjs`: simplify + TopoJSON + `shapeId`. Commit output.
- `build-areacodes.mjs`: download NANPA CSV, produce `areacodes.json`, fail on
...
| Risk | Mitigation |
|------|-----------|
| Stale geometry as new splits happen | Build-time check for unmapped NPAs; documented manual shape refresh; centroid fallback renderer |
...
- **npm over yarn**, **drop nix**. Purely to shrink the toolchain; easy to undo.
...
1. Scaffold Vite + React + TS in this repo; delete the old toolchain files.
2. Add lint/typecheck/test/deploy workflows; enable GitHub Pages on the repo.
3. Write `scripts/build-shapes.mjs` and check the first TopoJSON output.
4. Fetch the NANPA NPA report and write `scripts/build-areacodes.mjs`.
